...
class CNN(object):
    ...
    def __init__(
      ...
      filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        r1 = self.create_tower(embedded_x1, sequence_length, embedding_size, filter_sizes, num_filters, False)
        r2 = self.create_tower(embedded_x2, sequence_length, embedding_size, filter_sizes, num_filters, True)

        # Final (unnormalized) scores and predictions
        with tf.name_scope("output"):
            # Cosine similarity
            # prod = tf.reduce_sum(tf.mul(r1, r2), 1, True)

            # r1_norm = tf.sqrt(tf.reduce_sum(tf.square(r1), 1, True))
            # r2_norm = tf.sqrt(tf.reduce_sum(tf.square(r2), 1, True))
            # norm_prod = tf.mul(r1_norm, r2_norm)

            # cos_sim = tf.truediv(prod, norm_prod)
            # self.scores = cos_sim
            # self.predictions = tf.cast(tf.rint(self.scores), tf.int64, name="predictions")
